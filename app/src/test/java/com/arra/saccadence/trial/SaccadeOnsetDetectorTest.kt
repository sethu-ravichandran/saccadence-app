...
class SaccadeOnsetDetectorTest {
    ...
    @Test
    fun `displacement threshold from a quiet baseline is small but above the noise floor`() {
        val threshold = detector.displacementThresholdFrom(quietBaseline())
        assertTrue("threshold should be positive and bounded: $threshold", threshold in 0.1..5.0)
    }
    ...
    fun `rejects when no movement crosses threshold within the window`() {
        ...
        // Stimulus placed past the start of the trace so the detector has a
        // pre-step baseline to compare against — a step with no quiet stretch
        // before it is a different rejection ("no_pre_step_baseline").
        val stimulus = StepStimulus(targetIndex = 1, stimulusPhoneTimeMs = 400.0, stepAmplitudeDeg = 12.0, windowMs = 600.0)
